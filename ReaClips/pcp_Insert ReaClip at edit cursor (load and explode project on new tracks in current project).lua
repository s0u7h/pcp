...
function InsertMediaItemAndExplodeInNewTab()
  ...
  pos = reaper.GetCursorPosition()-- get the edit cursor position
  bpm = reaper.GetTempoTimeSigMarker(0,0)
  ...
  reaper.SetTempoTimeSigMarker(0, -1, bpm)
  ...
  -- sel_length = end_time - start_time -- length of time selection
  endpos = pos + end_time
  
  reaper.SetEditCurPos(endpos, 0, 0)
  reaper.Main_OnCommand(40396, 0)  -- Time selection: Move contents of time selection to edit cursor (moving later items)
